...
const RegisterFormPage = ({ setUserList, setIsLogin }) => {
  const {
    register,
    watch,
    setError,
  ...
  const users = [...usersCtx.users, ...usersCtx.admin];

  const onSubmit = values => {
    // const emailIndex = users.findIndex(item => item.email === values.email);
    // if (emailIndex !== -1) {
    //   setError('email', {
    //     type: 'manual',
    ...
    //   });
    // } else {
    //   return usersCtx.addUsers(values)
    //   }
